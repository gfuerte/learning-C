...
int searchlow(struct treenode* root);
...
bool check(struct treenode* root, int value) {
	bool result = false;
		...
		if(root->value == value) {
			...
			if (value > root->value) {
				result = check(root->right, value);
			} else if (value < root->value) {
				result = check(root->left, value);
			}
		}	
	}
